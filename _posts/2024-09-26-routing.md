...
**Lemma 2.** Fix any node $$v$$. Let the route from $$v$$ to $$\pi(v)$$ follow the sequence of edges $$(e_1, \ldots, e_k)$$. Let $S$ be the set of all other nodes $$v^{\prime} \in \{0,1\}^n$$ whose routes pass through at least one edge in $$\{e_1, \ldots, e_k\}$$. Then the delay incurred by $$p_v$$ is at most $$\abs{S}$$.
