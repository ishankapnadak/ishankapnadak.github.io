...
1. Two sets $$A$$ and $$B$$ are **equicardinal** ($$\lvert A \rvert = \lvert B \rvert$$) if there is a bijection from $$A$$ to $$B$$.
2. $$B$$ has cardinality greater than or equal to $$A$$ ($$\lvert A \rvert \geq \lvert B \rvert$$) if there is an injection from $$A$$ to $$B$$.
3. $$B$$ has cardinality strictly greater than $$A$$ ($$\lvert A \rvert > \lvert B \rvert$$) if there is an injection but no bijection from $$A$$ to $$B$$.  

With this in mind, we define countable sets as follows. A set $$E$$ is said to be \textbf{countably infinite} if $$E$$ and $$\mathbb{N}$$ are equicardinal. A set is said to be countable if it is either finite or countable infinite. Equivalently, a set $$E$$ is countable if $$\lvert E \rvert \leq \lvert \mathbb{N} \rvert$$. (Why?) 
